...
- Offline handling: 3 consecutive failed state polls → red
  "reconnecting" banner, greyed panels, chart kept as-is; polling
  self-heals.
- Screensaver prevented while the chart screen shows
  (`isIdleTimerDisabled`).
- Day / dusk / night display modes (bottom-right button, persisted):
  dusk dims, night dims harder with a red tint. Plain alpha overlays —
  blend modes over UIKit-backed views are unreliable on tvOS.
- Layered app icon (parallax: navy contour background + red arrow) and
  top shelf images, generated by a CoreGraphics script.
