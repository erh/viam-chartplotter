...
**Accept.**
- [x] Tablet in landscape shows chart + persistent data panel without a drawer.
- [x] Phone is unchanged.
- [x] Rotating a tablet doesn't lose state.
...
| B3 | Areas from `area` components | M | ✅ **done** — discovery probe, areaVisibleToday season windows (wrap-aware), per-area toggles seeded from season (not persisted), ui_folder grouping from the H6 config fetch. |
