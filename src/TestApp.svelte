<script lang="ts">
  ...
  // Map API reference
  let _mapApi = $state<{ fitToVisibleBoats: () => void }>();
  ...
  onMount(() => {
    const interval = setInterval(() => {
      ...
      myBoat = {
        ...myBoat,
        location: [myBoat.location[0] + myLatDelta, myBoat.location[1] + myLngDelta] as [
          number,
          number,
        ],
      ...
      fleetBoats = fleetBoats.map((boat) => {
        ...
        return {
          ...boat,
          location: [boat.location[0] + latDelta, boat.location[1] + lngDelta] as [number, number],
...
<div class="test-container">
  <div class="header">
    ...
    <p class="subtitle">
      Simulates both <strong>viam-chartplotter</strong> (single connected boat with live GPS) and
      <strong>ais fleet</strong> (multiple boats with 30-day track history)
    </p>
  </div>
